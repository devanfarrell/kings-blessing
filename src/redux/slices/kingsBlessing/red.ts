...
const clearSelectedAnswers = redSlice.createAction("clearSelectedAnswers", draft => {
  [...draft.presentationOrder, "king", "queen"].forEach(fieldKey => {
    draft[fieldKey].forEach(circle =>
      circle.forEach((slice, i) => {
        if (slice === Selection.selected) {
...
export const redSelector: any = createSelector([rawSelector], (state: Progress) => state);

export const selectRedKing: any = createSelector([redSelector], (state: Progress) => state.king);

export const selectRedQueen: any = createSelector([redSelector], (state: Progress) => state.queen);

export const selectRedField = createSelector([redSelector], ({ cows, wheat, lumber, pigs, fruit, water, wool }) => ({
  cows,
  wheat,
  lumber,
  pigs,
...
export const selectPresentationOrder = createSelector([redSelector], (state: Progress) => state.presentationOrder);
...
export const submitRedAnswer = redSlice.createSideEffect("submitAnswerRed", function*() {
